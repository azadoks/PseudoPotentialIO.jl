...
"""
    _read_until(io, tag::AbstractString)

Read until a line contains `tag`.
"""
function _read_until(io::IO, tag::AbstractString)
    while true
...
"""
    _read_mesh_data(T::Type, io::IO, n::Integer)

Read whitespace-separated data from `io` as type `T` until `n` elements are read.
"""
function _read_mesh_data(T::Type, io::IO, n::Integer)
    mesh_data = T[]
...
"""
    _has_tag(io::IO, tag::AbstractString)

Check if `tag` occurs in `io` after the current position in `io`.
"""
function _has_tag(io::IO, tag::AbstractString)::Bool
    pos = position(io)
    _has_tag = false
    try
        _read_until(io, tag)
...
function parse_header_upf1!(io::IO, upf::Dict)
    ...
    header["has_so"] = _has_tag(io, "<PP_ADDINFO>")
    header["has_gipaw"] = _has_tag(io, "<PP_GIPAW_RECONSTRUCTION_DATA>")
    header["q_with_l"] = _has_tag(io, "<PP_QIJ_WITH_L>")
...
function parse_radial_grid_upf1!(io::IO, upf::Dict)
    _read_until(io, "<PP_R>")
    upf["radial_grid"] = _read_mesh_data(Float64, io, upf["header"]["mesh_size"])

    _read_until(io, "<PP_RAB>")
    upf["radial_grid_derivative"] = _read_mesh_data(Float64, io, upf["header"]["mesh_size"])
...
function parse_nlcc_upf1!(io::IO, upf::Dict)
    if upf["header"]["core_correction"]
        _read_until(io, "<PP_NLCC>")
        upf["core_charge_density"] = _read_mesh_data(Float64, io, upf["header"]["mesh_size"])
    end
end
...
function parse_local_upf1!(io::IO, upf::Dict)
    _read_until(io, "<PP_LOCAL>")
    upf["local_potential"] = _read_mesh_data(Float64, io, upf["header"]["mesh_size"])
end
...
function parse_beta_projectors_upf1!(io::IO, upf::Dict)
    _read_until(io, "<PP_NONLOCAL>")
    beta_projectors = []
    for i = 1:upf["header"]["number_of_proj"]
        _read_until(io, "<PP_BETA>")
        ...
        beta["radial_function"] = _read_mesh_data(Float64, io, beta["cutoff_radius_index"])
...
#             qij = _read_mesh_data(Float64, io, upf["header"]["mesh_size"])

#             _read_until(io, "<PP_QFCOEF>")
#             q_coeffs = _read_mesh_data(Float64, io, num_q_coeff * (2 * upf["header"]["l_max"] + 1))
...
function parse_pswfc_upf1!(io::IO, upf::Dict)
    ...
    for i = 1:upf["header"]["number_of_wfc"]
        ...
        wfc["radial_function"] = _read_mesh_data(Float64, io, upf["header"]["mesh_size"])
...
function parse_rhoatom_upf1!(io::IO, upf::Dict)
    _read_until(io, "<PP_RHOATOM>")
    upf["total_charge_density"] = _read_mesh_data(Float64, io, upf["header"]["mesh_size"])
end
