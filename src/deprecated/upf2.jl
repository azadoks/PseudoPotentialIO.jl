...
_parse_bool(s::AbstractString) = occursin("T", uppercase(s)) ? true : false
_parse_bool(s::Char) = uppercase(s) == 'T' ? true : false

function _get_attr(::Type{T}, node::EzXML.Node, key; default=zero(T)) where {T <: Number}
    return haskey(node, key) ? parse(T, strip(node[key])) : default
end

function _get_attr(::Type{T}, node::EzXML.Node, key; default=T("")) where {T <: AbstractString}
    return haskey(node, key) ? T(strip(node[key])) : default
end

function _get_attr(::Type{Bool}, node::EzXML.Node, key; default=false)
    return haskey(node, key) ? _parse_bool(strip(node[key])) : default
end

function _get_content(::Type{T}, node::EzXML.Node, dims...) where {T <: Number}
...
function _get_content(::Type{T}, node::EzXML.Node, key, dims...) where {T <: Number}
    target_node = findfirst(node, key)
    content = ismissing(target_node) ? nothing : _get_content(T, target_node, dims...)
    return content
end
...
function parse_header_upf2!(doc_root::EzXML.Node, upf::Dict)
    header = Dict()
    # version = _get_attr(String, doc_root, "version")
    node = findfirst("PP_HEADER", doc_root)
    header["format_version"] = 2
    header["generated"] = _get_attr(String, node, "generated")
    header["author"] = _get_attr(String, node, "author")
    header["date"] = _get_attr(String, node, "date")
    header["comment"] = _get_attr(String, node, "comment")
    header["element"] = _get_attr(String, node, "element")
    header["pseudo_type"] = _get_attr(String, node, "pseudo_type")
    header["relativistic"] = _get_attr(String, node, "relativistic")
    header["is_ultrasoft"] = _get_attr(Bool, node, "is_ultrasoft")
    header["is_paw"] = _get_attr(Bool, node, "is_paw")
    header["is_coulomb"] = _get_attr(Bool, node, "is_coulomb")
    header["has_so"] = _get_attr(Bool, node, "has_so")
    header["has_wfc"] = _get_attr(Bool, node, "has_wfc")
    header["has_gipaw"] = _get_attr(Bool, node, "has_gipaw")
    header["paw_as_gipaw"] = _get_attr(Bool, node, "paw_as_gipaw")
    header["core_correction"] = _get_attr(Bool, node, "core_correction")
    header["functional"] = _get_attr(String, node, "functional")
    header["z_valence"] = _get_attr(Float64, node, "z_valence")
    header["total_psenergy"] = _get_attr(Float64, node, "total_psenergy")
    header["wfc_cutoff"] = _get_attr(Float64, node, "wfc_cutoff")
    header["rho_cutoff"] = _get_attr(Float64, node, "rho_cutoff")
    header["l_max"] = _get_attr(Int, node, "l_max")
    header["l_max_rho"] = _get_attr(Int, node, "l_max_rho")
    header["l_local"] = _get_attr(Int, node, "l_local")
    header["mesh_size"] = _get_attr(Int, node, "mesh_size")
    header["number_of_wfc"] = _get_attr(Int, node, "number_of_wfc")
    header["number_of_proj"] = _get_attr(Int, node, "number_of_proj")
...
function parse_radial_grid_upf2!(doc_root::EzXML.Node, upf::Dict)
    ...
    dx = _get_attr(Float64, node, "dx")
    mesh = _get_attr(Int, node, "mesh")
    xmin = _get_attr(Float64, node, "xmin")
    rmax = _get_attr(Float64, node, "rmax")
    zmesh = _get_attr(Float64, node, "zmesh")

    (mesh_type, mesh_a, mesh_b) = _guess_mesh_type(upf["radial_grid"], upf["radial_grid_derivative"])
...
function parse_beta_projectors_upf2!(doc_root::EzXML.Node, upf::Dict)
    ...
    for i = 1:upf["header"]["number_of_proj"]
        ...
        beta["label"] = _get_attr(String, node, "label")
        beta["angular_momentum"] = _get_attr(Int, node, "angular_momentum")
        ir_cut = _get_attr(Int, node, "cutoff_radius_index")
        beta["cutoff_radius_index"] = ir_cut
        beta["cutoff_radius"] = _get_attr(Float64, node, "cutoff_radius")
        beta["index"] = _get_attr(Int, node, "index")
        ...
        if upf["header"]["has_so"]
            ...
            beta["index"] = _get_attr(Int, node_so, "index")
            beta["angular_momentum"] = _get_attr(Int, node_so, "lll")
            beta["total_angular_momentum"] = _get_attr(Float64, node_so, "jjj")
        end
...
#         q_with_l = _get_attr(Bool, node, "q_with_l")
...
#                         Qij["i"] = i
#                         Qij["j"] = j
#                         Qij["angular_momentum"] = _get_attr(Int, node, "angular_momentum")
...
#     else
#         node = findfirst("PP_NONLOCAL/PP_AUGMENTATION", doc_root)
#         upf["header"]["cutoff_radius_index"] = _get_attr(Int, node, "cutoff_r_index")
...
#             wfc["angular_momentum"] = _get_attr(Int, node_wfc, "l")
#             wfc["label"] = _get_attr(String, node_wfc, "label")
#             wfc["index"] = _get_attr(Int, node_wfc, "index")
...
#         paw_core_energy = _get_attr(Float64, node_paw, "core_energy"; default=missing)
...
function parse_pswfc_upf2!(doc_root::EzXML.Node, upf::Dict)
    ...
    for i = 1:upf["header"]["number_of_wfc"]
        ...
        wfc = Dict()
        wfc["label"] = _get_attr(String, node, "label")
        wfc["angular_momentum"] = _get_attr(Int, node, "l")
        wfc["occupation"] = _get_attr(Float64, node, "occupation")
        wfc["pseudo_energy"] = _get_attr(Float64, node, "pseudo_energy")  # Ry
        wfc["index"] = _get_attr(Int, node, "index")
        wfc["radial_function"] = parse.(Float64, split(strip(node.content)))
        if upf["header"]["has_so"]
            node_so = findfirst("PP_SPIN_ORB/PP_RELWFC.$i", doc_root)
            wfc["index"] = _get_attr(Int, node_so, "index")
            wfc["angular_momentum"] = _get_attr(Int, node_so, "lchi")
            wfc["total_angular_momentum"] = _get_attr(Float64, node_so, "jchi")
            wfc["principal_quantum_number"] = _get_attr(Int, node_so, "nn")
        end
...
#             upf["beta_projectors"][i]["angular_momentum"] = _get_attr(Float64, node, "lll")
#             upf["beta_projectors"][i]["total_angular_momentum"] = _get_attr(Float64, node, "jjj")
#         end

#         # for i = 1:upf["header"]["number_of_wfc"]
#         #     node = findfirst("PP_SPIN_ORB/PP_RELWFC.$i", doc_root)
#         #     upf["paw_data"]["ae_wfc"]["ae_wfc_rel"] = parse(Float64, strip(node.content))
#         #     upf["paw_data"]["ae_wfc"]["total_angular_momentum"] = _get_attr(Float64, node, "jchi")
#         #     upf["paw_data"]["ps_wfc"]["total_angular_momentum"] = _get_attr(Float64, node, "jchi")
#         # end
#     end
# end
