...
function HghPsP(file::HghFile)
    ...
    return HghPsP(Zatom, sum(Float64.(file.zion)), Int(file.lmax), file.rloc, cloc, file.rp, file.h)
end
...
"""
@inline function local_potential_polynomial_fourier(psp::HghPsP, x=Polynomial([0., 1.]))
    rloc = psp.rloc
    Zval = psp.Zval
    ...
    4π * rloc^2 * (-Zval + sqrt(π / 2) * rloc * x^2 * P)
end
...
function local_potential_fourier(psp::HghPsP, q::T) where {T <: Real}
    x = q * psp.rloc
    local_potential_polynomial_fourier(psp, x) * exp(-x^2 / 2) / x^2
end
...
"""
function qcut_psp_local(psp::HghPsP{T}) where {T}
    Q = local_potential_polynomial_fourier(T, psp)  # polynomial in t = q * rloc
...
"""
@inline function projector_polynomial_fourier(psp::HghPsP, l, n, t=Polynomial([0., 1.]))
    ...
    (l == 0 && n == 1) && return convert(typeof(t), common)
    (l == 0 && n == 2) && return common * 2 /  sqrt(  15)       * ( 3 -   t^2      )
    (l == 0 && n == 3) && return common * 4 / 3sqrt( 105)       * (15 - 10t^2 + t^4)
    #
    (l == 1 && n == 1) && return common * 1 /  sqrt(   3) * t
    (l == 1 && n == 2) && return common * 2 /  sqrt( 105) * t   * ( 5 -   t^2)
    (l == 1 && n == 3) && return common * 4 / 3sqrt(1155) * t   * (35 - 14t^2 + t^4)
    #
    (l == 2 && n == 1) && return common * 1 /  sqrt(  15) * t^2
    (l == 2 && n == 2) && return common * 2 / 3sqrt( 105) * t^2 * ( 7 -   t^2)
    #
    (l == 3 && n == 1) && return common * 1 /  sqrt( 105) * t^3

    error("Not implemented for l=$l and i=$n")
end
...
"""
function qcut_psp_projector(psp::HghPsP{T}, l, n) where {T}
    Q = projector_polynomial_fourier(psp, l, n)  # polynomial in q * rnl[l + 1]

    # Find the roots of the derivative polynomial:
    res = roots(derivative(Q) - Polynomial([0, 1]) * Q)
    res = T[r for r in res if abs(imag(r)) < 100eps(T)]
    maximum(res, init=zero(T)) / psp.rnl[l + 1]
end


# [HGH98] (7-15) except they do it with plane waves normalized by 1/sqrt(Ω).
function projector_fourier(psp::HghPsP, l, n, q::T) where {T <: Real}
    x::T = q * psp.rnl[l + 1]
    projector_polynomial_fourier(psp, l, n, x) * exp(-x^2 / 2)
end


# [HGH98] (3)
function projector_real(psp::HghPsP, l, n, r::T) where {T <: Real}
    rnl = T(psp.rnl[l + 1])
    ired = (4n - 1) / T(2)
    sqrt(T(2)) * r^(l + 2(n - 1)) * exp(-r^2 / 2rnl^2) / rnl^(l + ired) / sqrt(gamma(l + ired))
end
